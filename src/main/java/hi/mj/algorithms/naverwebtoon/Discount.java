...
public class Discount {
    public int solution(int[] prices, int[] discounts) {
        ...
        List<Integer> priceList = stream(prices).boxed().collect(Collectors.toList());
        List<Integer> discountsList = stream(discounts).boxed().collect(Collectors.toList());
        orderingDESC(priceList, discountsList);

        for(int i = 0; i < priceList.size(); i++) {
            int price = priceList.get(i);

            if(applicableDiscount(discounts, i)) {
                Integer discount = discountsList.get(i);
    ...
    private boolean applicableDiscount(int[] discounts, int i) {
        return i <= discounts.length - 1;
    }

    private void orderingDESC(List<Integer> priceList, List<Integer> discountsList) {
        priceList.sort(Collections.reverseOrder());
        discountsList.sort(Collections.reverseOrder());
    }
}
